...
pub const BSP = struct {
    verts: []Vertex, edges: []Edge, sedges: []i32, faces: []Face, planes: []Plane, nodes: []Node,
    models: []Model, clips: []ClipNode, leafs: []Leaf, spawn: ?Vec3, a: std.mem.Allocator,
    ...
    pub fn load(a: std.mem.Allocator, path: []const u8) !BSP {
        const f = try std.fs.cwd().openFile(path, .{}); defer f.close();
        var h: Header = undefined; _ = try f.read(std.mem.asBytes(&h));
        if (h.ver != 29) return error.BadVer;
        return .{
            .verts = try rd(Vertex, a, f, h.lumps[3]), .edges = try rd(Edge, a, f, h.lumps[12]),
            .sedges = try rd(i32, a, f, h.lumps[13]), .faces = try rd(Face, a, f, h.lumps[7]),
            .planes = try rd(Plane, a, f, h.lumps[1]), .nodes = try rd(Node, a, f, h.lumps[5]),
            .models = try rd(Model, a, f, h.lumps[14]), .clips = try rd(ClipNode, a, f, h.lumps[9]),
            .leafs = try rd(Leaf, a, f, h.lumps[10]), .spawn = try getSpawn(a, f, h.lumps[0]), .a = a
        };
    }

    pub fn mesh(self: *const BSP, a: std.mem.Allocator) !Mesh {
        var v = std.ArrayListUnmanaged(f32){}; var idx = std.ArrayListUnmanaged(u32){};
        var m = std.AutoHashMap(u16, u32).init(a); defer m.deinit();
        for (self.faces) |face| {
            ...
            while (j < face.nedges) : (j += 1) {
                ...
                try fv.append(a, if (se > 0) self.edges[@intCast(se)].v[0] else self.edges[@intCast(-se)].v[1]);
            }
            if (fv.items.len >= 3) for (fv.items[1 .. fv.items.len - 1], 0..) |v1, k| {
                for ([_]u16{ fv.items[0], v1, fv.items[k + 2] }) |vi| if (!m.contains(vi)) {
                    try m.put(vi, @intCast(m.count())); const vx = self.verts[vi];
    ...
    pub fn trace(self: *const BSP, s: Vec3, e: Vec3, b: Box) Trace {
        var t = Trace{};
        if (self.clips.len == 0) return t;
        const ext = Vec3.new((b.max.x() - b.min.x()) * 0.5, (b.max.y() - b.min.y()) * 0.5, (b.max.z() - b.min.z()) * 0.5);
        self.traceR(0, 0, 1, s, e, ext, &t);
        return t;
    }

    fn traceR(self: *const BSP, i: i32, sf: f32, ef: f32, s: Vec3, e: Vec3, ext: Vec3, t: *Trace) void {
        if (i < 0) {
            if (i != -1 and self.leafs.len > 0) {
                const l = self.leafs[@intCast(-(i + 1))];
                if (l.ty == -2) t.solid = true;
            }
            return;
        }
        const n = self.clips[@intCast(i)]; const p = self.planes[@intCast(n.plane)];
        ...
        if (d1 != d2 and (d1 >= 0) != (d2 >= 0)) {
            const f = d1 / (d1 - d2); mf = sf + (ef - sf) * f;
        ...
        if (t.solid and side == 0 and mf < t.frac) {
            t.frac = mf; t.n = Vec3.new(p.n[0], p.n[1], p.n[2]); t.hit = true;
        }
    }
    ...
    pub fn slide(self: *const BSP, p: Vec3, v: Vec3, dt: f32, b: Box) Vec3 {
        ...
        for (0..4) |_| {
            ...
            if (tr.hit) {
                ...
                vel = Vec3.new(vel.x() - tr.n.x() * bk, vel.y() - tr.n.y() * bk, vel.z() - tr.n.z() * bk);
            }
            t *= 1 - tr.frac;
            if (t < 0.001) break;
        }
...
fn getSpawn(a: std.mem.Allocator, f: std.fs.File, l: Lump) !?Vec3 {
    ...
    const d = try a.alloc(u8, @intCast(l.len)); defer a.free(d);
    try f.seekTo(@intCast(l.ofs)); _ = try f.read(d);
    var it = std.mem.splitScalar(u8, d, '\n'); var o: ?Vec3 = null;
    while (it.next()) |ln| {
        const t = std.mem.trim(u8, ln, " \t\r");
        if (std.mem.indexOf(u8, t, "{") != null) o = null;
        if (std.mem.indexOf(u8, t, "\"origin\"") != null)
