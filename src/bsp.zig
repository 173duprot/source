...
pub const BSP = struct {
    verts: []Vertex, edges: []Edge, sedges: []i32, faces: []Face, planes: []Plane, nodes: []Node, models: []Model, spawn: ?Vec3, a: std.mem.Allocator,

    pub fn deinit(self: *BSP) void {
        inline for (.{ self.verts, self.edges, self.sedges, self.faces, self.planes, self.nodes, self.models }) |d| self.a.free(d);
    }
    ...
    pub fn load(a: std.mem.Allocator, path: []const u8) !BSP {
        ...
        return .{
            ...
            .models = try rd(Model, a, f, h.lumps[14]), .spawn = try getSpawn(a, f, h.lumps[0]), .a = a
        };
    }
...
fn getSpawn(a: std.mem.Allocator, f: std.fs.File, l: Lump) !?Vec3 {
    ...
    var in = false; var it = std.mem.splitScalar(u8, d, '\n');
    while (it.next()) |ln| {
        const t = std.mem.trim(u8, ln, " \t\r");
        in = in or std.mem.indexOf(u8, t, "\"classname\" \"info_player_start\"") != null;
        if (in and std.mem.indexOf(u8, t, "\"origin\"") != null)
