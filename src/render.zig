...
pub const Renderer = struct {
    ...
    pub fn init(m: Mesh, clr: [4]f32) Renderer {
        sg.setup(.{ .environment = sglue.environment(), .logger = .{ .func = sokol.log.func } });
        return .{
            .bind = .{
                .vertex_buffers = .{ sg.makeBuffer(.{ .data = sg.asRange(m.verts) }), .{}, .{}, .{}, .{}, .{}, .{}, .{} },
                .index_buffer = sg.makeBuffer(.{ .usage = .{ .index_buffer = true }, .data = sg.asRange(m.indices) }),
            },
            .pass = .{ .colors = .{ .{ .load_action = .CLEAR, .clear_value = .{ .r = clr[0], .g = clr[1], .b = clr[2], .a = clr[3] } }, .{}, .{}, .{}, .{}, .{}, .{}, .{} } },
            .count = @intCast(m.indices.len),
            .itype = .UINT16,
        };
    }
